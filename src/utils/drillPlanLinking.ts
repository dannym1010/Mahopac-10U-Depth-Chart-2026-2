...
export function findMatchingWhiteboardDrill(
  ...
  const aliasMap: Record<string, string[]> = {
    'ball on a stick': ['ball on stick', 'get off', 'cadence', 'ball-on-a-stick'],
    'krausko blitz master': ['krausko', 'blitz master', 'krausko pad', 'blitz pod'],
    'read step pod': ['read step', 'read-step', 'lb read', 'keys pod'],
    'gap sound run fit pod': ['gap sound', 'run fit', 'run fits', 'gap-sound'],
    'form tackling circuit': ['form tackle', 'form tackling', 'tackle circuit', 'tackling pod'],
    'angle tackling on bags': ['angle tackle', 'angle tackling', 'tackle bags', 'bag tackle'],
    'shed and tackle circuit': ['shed & tackle', 'shed and tackle', 'block shed', 'shedding'],
    'pursuit & strip to the cone': ['pursuit & strip', 'pursuit', 'strip to cone', 'turnover circuit', 'strip ball'],
    'cover 3 match': ['cover 3', 'cov 3', 'c3 match'],
    'cover 2 hard corner': ['cover 2', 'cov 2', 'c2 hard corner', 'hard flat'],
    'slant & angle (pinch / pirate)': ['slant', 'pinch', 'pirate', 'slant & angle', 'd-line stunt'],
  };
